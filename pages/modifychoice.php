...
<div id="templatemo_main">
	...
	<p>Select the Department</p>
		...
		do{
			...
			<option value="<?php echo $results['deptID']; ?>"><?php echo $results['deptName'];?></option>
			<?php
...
if($cvalue==1){
			...
			$Qre="SELECT * FROM staff WHERE departmentId=1";
			$Qque=mysql_query($Qre);
			$re=mysql_fetch_array($Qque);
do{
		echo "<tr>";
			echo "<td>".$re['Fname']."</td><td>".$re['Mname']."</td><td>".$re['Lname']."</td><td>".$re['staffno']."</td><td>".$re['departmentId']."</td><td>".$re['position']."</td><td>".$re['address']."</td><td>".$re['email']."</td><td>".$re['telephone']."</td><td>
        <a href=http://localhost/COLS1/pages/edit1.php?id=".$re['Id'].">Edit</a>&nbsp;&nbsp;&nbsp;
        <a href=delete.php?id=".$re['Id']." class='delete' >Delete</a></td>";
		echo "</tr>";
}while($re=mysql_fetch_array($Qque));	
}elseif($cvalue==2){

					echo "<table border=1>";
			echo "<td>First Name</td><td>Middle Name </td><td>Last Name</td><td>Staff No</td><td>Department</td><td>Position</td><td>Adress</td><td>Email</td><td>Telephone Number</td><td>Actions</td>";
			$Qre="SELECT * FROM staff WHERE departmentId=2";
			$Qque=mysql_query($Qre);
			$re=mysql_fetch_array($Qque);
do{
		echo "<tr>";
		...
		echo "</tr>";
					...
					echo "<table border=1>";
			echo "<td>First Name</td><td>Middle Name </td><td>Last Name</td><td>Staff No</td><td>Department</td><td>Position</td><td>Adress</td><td>Email</td><td>Telephone Number</td><td>Actions</td>";
			$Qre="SELECT * FROM staff WHERE departmentId=3";
			$Qque=mysql_query($Qre);
			$re=mysql_fetch_array($Qque);
do{
		echo "<tr>";
		...
		echo "</tr>";
					...
					echo "<table border=1>";
			echo "<td>First Name</td><td>Middle Name </td><td>Last Name</td><td>Staff No</td><td>Department</td><td>Position</td><td>Adress</td><td>Email</td><td>Telephone Number</td><td>Actions</td>";
			$Qre="SELECT * FROM staff WHERE departmentId=4";
			$Qque=mysql_query($Qre);
			$re=mysql_fetch_array($Qque);
do{
		echo "<tr>";
