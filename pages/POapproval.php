...
$q=mysql_query("SELECT * FROM leavedetails where staffID='$staff_id'");
...
$staff=mysql_query("SELECT * FROM staff WHERE Id='{$staff_id}'");
...
<div id="templatemo_main">
	...
	<?php
	    ...
	    $sql = "SELECT * FROM leavestatus WHERE statusId='{$staff_id}'";
	   // die($sql);
		$status_check=mysql_query($sql);
		if(!$status_check){
			die("Query error: ".mysql_error($con));
		}
		$status_value=mysql_fetch_array($status_check);
		$status_array=mysql_num_rows($status_check);
		//die("Waht is inside status array ->".$status_array);
if($status_array<=0){
	...
	<form method="post" action="POapproval.php?no=<?php echo $id."& app= ".$app."& staff_id= ".$staff_id;?>">
	<fieldset>
